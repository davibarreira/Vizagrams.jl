...
function replot(
    ...
    encodings = NamedTupleTools.rec_merge(plt.encodings, enc)

    if rescale
        enc_no_scale = NamedTuple(
            map(zip(keys(plt.encodings), values(plt.encodings))) do (k, v)
                k => @delete v.scale
            end,
        )
...
function replot(plt1::Plot, plt2::Plot)
    encodings = NamedTupleTools.rec_merge(plt1.encodings, plt2.encodings)
