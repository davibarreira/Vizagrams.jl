using Vizagrams
import Vizagrams:
    MPrim, TM, MTDiagram, ζ, θ, compare_primitives, compare_trees, flatten, FreeComp
...
@testset "Mark Tree" begin
    @testset "Lifting Marks and Diagrams" begin
        ...
        @test compare_trees(
            dlift(Circle()),
            Pure{Vector{Prim}}(Prim[Prim{Circle}(Circle(1, [0, 0]), S(Dict{Any,Any}()))]),
        )
        ...
        @test compare_trees(
            ζ(mlift(Circle())),
            Pure{Mark}(MPrim{Circle}(Prim{Circle}(Circle(1, [0, 0]), S(Dict{Any,Any}())))),
        )
    end
    ...
    @testset "flatten, θ, ζ" begin
        ...
        @test all(
            map(
                ...
                zip(flatten(MyTestMark()), [Prim(Circle()), Prim(Square())]),
            ),
        )
        ...
        @test compare_primitives(
            fmap(x -> Square(), mlift(Prim(Circle())))._1, Prim(Square())
        )
    end
